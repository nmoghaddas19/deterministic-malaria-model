...
init_Evasb <- 0
init_Ivasb <- 0
initial(Svasb) <- init_Svasb * mv0
initial(Evasb) <- init_Evasb * mv0
initial(Ivasb) <- init_Ivasb * mv0
...
# Number of mosquitoes that become infected at each point in ASB compartments
ince_asb <- FOIv*Svasb
lag_incv_asb <- ince_asb*surv
incv_asb <- delay(lag_incv_asb, delayMos)
#incv_ih <- lag_incv_ih
